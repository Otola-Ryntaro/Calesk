...
Caleskは、Google Calendarの予定をデスクトップ壁紙として表示するアプリケーションです。
...
```powershell
Get-ScheduledTask -TaskName "CaleskUpdate"
```

またはGUIで：
1. `Win + R` → `taskschd.msc` を入力
2. タスクスケジューラライブラリで「CaleskUpdate」を確認
...
1. `com.user.calesk.plist`を編集
2. サービスを再起動：
   ```bash
   launchctl unload ~/Library/LaunchAgents/com.user.calesk.plist
   launchctl load ~/Library/LaunchAgents/com.user.calesk.plist
   ```

#### Windows

1. タスクスケジューラーを開く
2. 「CaleskUpdate」をダブルクリック
3. 「トリガー」タブで時刻を変更
