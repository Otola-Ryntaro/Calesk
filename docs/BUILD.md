...
ビルドが成功すると、`dist/CalendarWallpaper.app` が生成されます（約191MB）。
...
# 起動テスト
open dist/CalendarWallpaper.app
```
...
このスクリプトは以下を実行します：
1. `CalendarWallpaper.app` を `/Applications` にコピー
...
PyInstallerが特定のモジュールを検出できない場合、`CalendarWallpaper.spec` の `hiddenimports` リストに追加してください。
...
```bash
# 署名の確認
codesign -dv --verbose=4 /Applications/CalendarWallpaper.app

# ad-hoc署名の追加（PyInstallerが自動実行）
codesign --force --deep --sign - /Applications/CalendarWallpaper.app
```
...
```bash
# DMGイメージの作成
hdiutil create -volname "Calendar Wallpaper" \
  -srcfolder dist/CalendarWallpaper.app \
  -ov -format UDZO \
  CalendarWallpaper.dmg
```
...
codesign --deep --force --verify --verbose \
  ...
  /Applications/CalendarWallpaper.app

# 公証（Notarization）
xcrun notarytool submit CalendarWallpaper.dmg \
...
- `CalendarWallpaper.spec`: PyInstaller設定ファイル
...
```python
# CalendarWallpaper.spec の excludes に不要なモジュールを追加
excludes=[
    'pytest',
    'pytest-qt',
    'tkinter',  # 使用していない場合
...
pyinstaller CalendarWallpaper.spec --noconfirm
```
